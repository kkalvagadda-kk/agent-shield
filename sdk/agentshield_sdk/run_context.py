...
below the ContextVar section. See that file for the full rationale; `suite-99` asserts the
copies agree.

THIS PROCESS MUST NEVER MINT A ROOT CONTEXT — see the declarative-runner copy for why.

The SDK copy additionally OWNS the per-run ContextVars. They live here rather than in
`graph_builder.py` so `hitl.py` can read them without the circular import it currently
dances around: graph_builder imports hitl, so hitl cannot import graph_builder. A leaf
module with no project imports is the only place both can depend on.
"""
...
# ---------------------------------------------------------------------------
# Per-run ContextVars (SDK only)
# ---------------------------------------------------------------------------
# ContextVar, not a module global: one pod serves concurrent runs, and a global would
# leak one user's identity into another user's tool call — which OPA would then authorize.
...
def set_current(ctx: "RunContext | None") -> None:
    _current_run_context.set(ctx)


def current() -> "RunContext | None":
    """The RunContext for the run being served, or None if identity did not arrive.

    Returns None rather than an empty RunContext on purpose: a caller must be able to tell
    "no identity" from "a user whose sub happens to be blank". Conflating them is how
    `user_id=""` reaches OPA looking deliberate.
    """
    return _current_run_context.get()


def current_user_sub() -> str:
    ctx = current()
    return ctx.user_sub if ctx else ""


def current_actor_chain() -> list:
    ctx = current()
    return list(ctx.actor_chain) if ctx else []

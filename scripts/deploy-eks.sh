...
REGISTRY_API_TAG="0.2.230"   # 0.2.230: mcp_oauth path-aware PRM discovery (RFC 9728 §3.1 _prm_candidates) — GitHub-style path-scoped protected-resource metadata is found instead of 404ing to the AS fallback.   # 0.2.225: MCP-as-tool-source — mcp_servers router + migration 0072 + mcp-proxy client (matches values.yaml). 0.2.224: HITL reactive-chat approval-status fix (_chat_thread_id keyed by session_id). 0.2.221: Decision 30 — matches values.yaml
MCP_PROXY_TAG="0.1.5"   # 0.1.5: bound call_tool by MCP_CONNECT_TIMEOUT_SECONDS (revoked-token upstream 401 fails closed instead of hanging). 0.1.0: NEW centralized MCP wire client (tool discovery + governed tool calls). Dockerfile COPYs scripts/e2e/fixtures/stub_mcp_server.py → REPO-ROOT build context.
...
DECLARATIVE_RUNNER_TAG="0.1.63"   # 0.1.62: rebuilds sdk 0.2.5 (MCP arg-marshaling fix — drop omitted-optional None args; fixes Tavily). 0.1.61/0.1.59: prior (matches values.yaml declarativeRunnerTag)
STUDIO_TAG="0.1.163"   # 0.1.161: MCP-as-tool-source Studio UI (Phases 12-14). 0.1.160: human-grantee grant creation (Decision 30) + T024
SCHEDULER_TAG="0.1.1"

...
REGISTRY_API_TAG="0.2.228"   # 0.2.225: MCP-as-tool-source — mcp_servers router + migration 0072 + mcp-proxy client (matches values.yaml). 0.2.224: HITL reactive-chat approval-status fix (_chat_thread_id keyed by session_id). 0.2.221: Decision 30 — matches values.yaml

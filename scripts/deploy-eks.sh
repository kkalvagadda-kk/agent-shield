...
EVAL_RUNNER_TAG="0.1.15"   # 0.1.14: parity with deploy-cpe2e.sh.   # 0.1.10:
MINIO_CP1_TAG="0.1.0"
...
# (identity propagation P3, §4.5). One secret PER service so a leak impersonates one
# service, not all three. See deploy-cpe2e.sh for the full note.
...
kubectl create secret generic agentshield-encryption -n "$NS" \
  ...
  --dry-run=client -o yaml | kubectl apply -f - >/dev/null
# Run-context signing key — separate Secret from agentshield-encryption on purpose:
# different blast radius, independently rotatable. See deploy-cpe2e.sh for the full note.
kubectl create secret generic agentshield-run-context -n "$NS" \
...
# Consumed by realm-init-job (creates the clients) AND by the callers (mint tokens).
kubectl create secret generic agentshield-service-clients -n "$NS" \
...
# (services/registry-api/bootstrap_admin.py, Decision 40) — /ready is red until it
# succeeds, so a green rollout above already proves it. This step used to run
# scripts/seed-platform-admin-role.sh unconditionally; it must NOT, because the seed
# stamps assigned_by='system:seed-platform-admin' OVER the bootstrap's
# 'system:bootstrap' and destroys the provenance suite-97 T-S97-001/004 assert on —
# which is exactly how it was caught. The script is retained as a MANUAL repair tool
# (gap ledger G-R0-5); run it by hand only if the audit reports a stranded row:
#   kubectl exec -n "$NS" deploy/agentshield-registry-api -c registry-api -- python3 -c \
#     "import json,urllib.request;print(urllib.request.urlopen('http://localhost:8000/api/v1/admin/identity-audit').read().decode())"

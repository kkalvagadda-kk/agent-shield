#!/usr/bin/env bash
# Suite 49: Auth-config credential value validation
...
echo "=== Suite 49: Credential Validation ==="

# --------------------------------------------------------------------------
# T-S49-001 — Valid credential persists and stores the real value
# --------------------------------------------------------------------------
echo "T-S49-001 — Valid api_key credential is accepted + K8s secret holds real value"
run '
...
print("PASS: T-S49-001")
'

# --------------------------------------------------------------------------
# T-S49-002 — HTTP-error-shaped value is REJECTED (the actual bug), not stored
# --------------------------------------------------------------------------
echo "T-S49-002 — httpx 403 error string rejected with 422, nothing persisted"
run '
import httpx, uuid
c = httpx.Client(base_url="http://localhost:8000/api/v1")
...
print("PASS: T-S49-002")
'

# --------------------------------------------------------------------------
# T-S49-003 — Empty / too-long / multi-line inline values rejected
# --------------------------------------------------------------------------
echo "T-S49-003 — empty, oversized, and multi-line inline values rejected (422)"
run '
import httpx, uuid
c = httpx.Client(base_url="http://localhost:8000/api/v1")
...
try_bad("line1\nline2", "multiline")
print("PASS: T-S49-003")
'

# --------------------------------------------------------------------------
# T-S49-004 — mTLS PEM (legitimately multi-line + long) is still accepted
# --------------------------------------------------------------------------
echo "T-S49-004 — mtls multi-line PEM credential accepted"
run '
import httpx, uuid
c = httpx.Client(base_url="http://localhost:8000/api/v1")
...
print("PASS: T-S49-004")
'

# --------------------------------------------------------------------------
# T-S49-005 — Update path also rejects an error-shaped value
# --------------------------------------------------------------------------
echo "T-S49-005 — PUT with error-shaped value rejected, original secret preserved"
run '
import base64, httpx, uuid
from kubernetes import client, config
c = httpx.Client(base_url="http://localhost:8000/api/v1")
...
c.delete(f"/auth-configs/{cid}")
print("PASS: T-S49-005")
'

echo ""
echo "=== Suite 49 COMPLETE: 5/5 ==="

#!/usr/bin/env bash
# scripts/e2e/suite-84-tool-description-metadata.sh — tool description + picker metadata (API layer)
#
...
#
#   T-S84-001  POST a tool with a multi-line description -> 201 and the response echoes it byte-identical
#   T-S84-002  GET the tool by name -> the stored description still has every newline (round-trip)
#   T-S84-003  the description survives in the LIST payload too (what the tiles read)
#   T-S84-004  PATCH/PUT to a longer multi-line description -> re-GET returns the new value intact
#   T-S84-005  a long description (~4KB, 60 lines) is not truncated by the column
#   T-S84-006  list payload carries risk_level + type for every tool (tile metadata contract)
#   T-S84-007  a tool created without a description does not 500 and reads back as empty/None
#   T-S84-008  cleanup: delete every fixture this suite created
set -euo pipefail
...
if [ -z "$API_POD" ]; then
  echo "FAIL  T-S84-FIXTURE  |  no Running registry-api pod found"; exit 1
fi

echo "=== Suite 84: Tool description (multi-line) + picker tile metadata ==="
echo "    pod: $API_POD"
echo ""
RUN_TAG="s84_$(date +%s)"
...
ok("T-S84-FIXTURE-000 fetched platform-admin token")
...
def base_tool(name, **over):
    body = {
        "name": name,
        "display_name": "S84 Description Tool",
        "type": "http",
        "risk_level": "high",
        "owner_team": "platform",
...
try:
    # ---- T-S84-001 create with a multi-line description -------------------
    st, t = call("POST", "/api/v1/tools/", PT, base_tool(TOOL, description=MULTILINE))
    if st not in (200, 201):
        bad("T-S84-001 create tool", f"{st} {t}")
        print(f"=== Suite 84: PASS={PASS} FAIL={FAIL} ==="); raise SystemExit(1)
    if t.get("description") == MULTILINE:
        ok("T-S84-001 create echoes the multi-line description byte-identical")
    else:
        bad("T-S84-001 create echo", f"got {t.get('description')!r}")

    # ---- T-S84-002 round-trip: the newlines SURVIVED the DB ---------------
    st, g = call("GET", f"/api/v1/tools/{TOOL}", PT)
    desc = (g or {}).get("description")
    if st == 200 and desc == MULTILINE:
        ok("T-S84-002 GET returns the stored description with every newline intact")
    else:
        bad("T-S84-002 round-trip", f"{st} lines={None if desc is None else desc.count(chr(10))} {desc!r}")
    # The specific failure the textarea would mask: stored as a single line.
    if desc is not None and desc.count("\n") == 3:
        ok("T-S84-002b description still spans 4 lines (not collapsed to one)")
    else:
        bad("T-S84-002b newline count", f"{None if desc is None else desc.count(chr(10))}")

    # ---- T-S84-003 the LIST payload carries it too ------------------------
    ...
    if st == 200 and mine and mine[0].get("description") == MULTILINE:
        ok("T-S84-003 list payload carries the full multi-line description")
    else:
        bad("T-S84-003 list description", f"{st} found={len(mine)}")

    # ---- T-S84-004 update to a different multi-line value -----------------
    ...
    if st2 == 200 and (g2 or {}).get("description") == UPDATED:
        ok("T-S84-004 edited multi-line description round-trips (5 lines)")
    else:
        bad("T-S84-004 edit round-trip", f"update={st} get={st2} {(g2 or {}).get('description')!r}")

    # ---- T-S84-005 a long description is not truncated --------------------
    ...
    if st == 200 and got == LONG:
        ok(f"T-S84-005 long description ({len(LONG)} bytes / 60 lines) not truncated")
    else:
        bad("T-S84-005 truncation", f"sent={len(LONG)} got={len(got)}")

    # ---- T-S84-006 tile metadata contract ---------------------------------
    ...
    if st == 200 and mine and mine[0].get("risk_level") == "high" and mine[0].get("type"):
        ok("T-S84-006 list carries risk_level + type for the picker tiles")
    else:
        bad("T-S84-006 tile metadata", f"{st} mine={mine[:1]}")
    if not missing:
        ok("T-S84-006b every listed tool exposes a type")
    else:
        bad("T-S84-006b tools missing type", f"{missing[:5]}")

    # ---- T-S84-007 no description at all still works ----------------------
    ...
    if st in (200, 201) and st2 == 200 and not (g4 or {}).get("description"):
        ok("T-S84-007 a tool with no description creates and reads back empty (no 500)")
    else:
        bad("T-S84-007 no-description path", f"create={st} get={st2} {(g4 or {}).get('description')!r}")

finally:
    for name in (TOOL, TOOL_NODESC):
        call("DELETE", f"/api/v1/tools/{name}", PT)
    print("PASS  T-S84-008 cleanup complete"); PASS += 1

print(f"=== Suite 84: PASS={PASS} FAIL={FAIL} ===")

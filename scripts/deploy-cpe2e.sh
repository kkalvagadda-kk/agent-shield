...
SCHEDULER_TAG="0.1.2"   # 0.1.2: read-side defence — schedule query filters a.status=active / w.status=published. Defence in depth behind the write-side disarm; the scheduler and event-gateway are SEPARATE services, so filtering one leaves the other armed.
EVENT_GATEWAY_TAG="0.1.5"   # 0.1.5: read-side status filter in webhook_auth._TRIGGER_SQL

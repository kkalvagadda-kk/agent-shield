...
#   - declarative-runner:0.1.39 (Fix graph-build crash for python tools & arg-less HTTP tools — the governed tool wrapper's executor emitted a __signature__ with a parameter (`**kwargs` for python tools; the `params` catch-all for HTTP tools with no {{template}} vars) that was absent from __annotations__, so LangChain 1.x introspection (create_schema_from_function → pydantic validate_arguments → type_hints[name]) raised KeyError('kwargs')/KeyError('params') and the pod CrashLooped. Class fix in the SDK: (1) __annotations__ is now DERIVED from the signature params (_annotations_from_params) so the two can never drift — every param has a type hint; (2) python tools now build named, typed params from their registered input_schema (_params_from_input_schema, threaded via tool_resolver) — the same treatment HTTP tools get from {{template}} vars — falling back to an annotated `**kwargs` only when no input_schema is declared (e.g. calculator); (3) graph_builder injects the InjectedState graph_state param BEFORE any VAR_KEYWORD, since a kw-only param can't follow **kwargs (previously the injection silently failed for python tools, dropping HITL reasoning capture). Verified against real langchain-core 1.4.9 in-pod: python(no-schema)/python(schema)/http(template)/http(no-vars) all bind and keep graph_state out of the model-facing schema. New sdk/tests/test_tool_executor_schema.py (16 cases) + suite-54.)
...
# ── Image tags ────────────────────────────────────────────────────────────────
REGISTRY_API_TAG="0.2.155"
SAFETY_ORCHESTRATOR_TAG="0.1.3"
DEPLOY_CONTROLLER_TAG="0.1.35"
STUDIO_TAG="0.1.126"
EVAL_RUNNER_TAG="0.1.4"
DECLARATIVE_RUNNER_TAG="0.1.39"
...
echo "  → declarative-runner:${DECLARATIVE_RUNNER_TAG} (fix python/arg-less-HTTP tool graph-build KeyError: annotations derived from signature + input_schema-driven named params)"

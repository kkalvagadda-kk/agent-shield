...
STUDIO_TAG="0.1.164"   # 0.1.164: merge origin/main + MCP picker parity. main brings the browse-and-select tile drawers (shared PickerTile + TilePickerDrawer; selected show as removable chips), the multi-line tool Description, and the viewer->consumer role rename. On top: the drawer fetches the FULL catalog active-only (listAllTools pages on `total`; /tools/ caps limit at 200 and 54 of 82 live tools are MCP-sourced, so the old listTools(200) silently truncated and offered 13 deprecated tools), MCP tiles carry a source-server chip instead of the raw type string, and a source filter (All/Native/per-server) keeps native tools findable. 0.1.162: MCP Phase 2 (WS-A) — McpServerDetailPage Health panel (status pill, last_success_at, consecutive_failures, last_error, last_synced_at, list_changed_supported, identity note) + 15s refetch

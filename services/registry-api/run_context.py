"""RunContext — the ONE identity object a run carries, and its wire format.

Design: docs/design/identity-propagation-architecture.md §4.1 / §4.3.

WHAT PROBLEM THIS SOLVES
------------------------
When a run starts today, the human who authorized it is carried nowhere. The agent pod
asks OPA "may I call this tool" with `user_id=""`, and OPA Gate 6 — live and AND-ed into
`allow` since WS-2 (`agentshield.rego:22,101-108,116`) — denies. That is a CURRENT
production denial, not a future risk, and it is why identity is sequenced ahead of the
remaining RBAC phases.

WHY AN HMAC TOKEN AND NOT THE KEYCLOAK JWT
------------------------------------------
Internal traffic (registry-api -> declarative-runner -> SDK pod) is its own trust domain,
already bounded by network policy. Verifying a Keycloak JWT at every hop would need JWKS
network calls inside declarative-runner and every SDK pod — `python-jose` is a
registry-api-only dependency and those components have no such infra. An HMAC token
verified in-process against a shared secret is zero-network-call and reuses the K8s Secret
pattern `AGENTSHIELD_ENCRYPTION_KEY` already established.

The raw Keycloak JWT stays authoritative at the true edges (`auth_middleware.require_user`)
and for verified SERVICE identity (§4.5). This token is for propagation only. It is NOT a
substitute for authentication: minting one requires having already authenticated somebody.

THREE COPIES, DELIBERATELY
--------------------------
No shared package spans `services/*` and `sdk/` — each vendors its own dependencies. So
this module is copied to `services/declarative-runner/run_context.py` and
`sdk/agentshield_sdk/run_context.py`, kept byte-identical below the import line. Only
registry-api may `mint` a root context; the other two verify and extend. `suite-99` asserts
the three copies agree, because three copies that drift are worse than one that is awkward.
"""

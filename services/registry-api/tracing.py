...
def fetch_trace_cost_tokens(trace_id: str) -> dict | None:
    """Sum cost + token usage from a trace's GENERATION observations.

    Langfuse computes per-LLM-call cost (``calculatedTotalCost``) and token
    counts on every ``type=GENERATION`` observation once the OTEL spans are
    ingested. A single run can make several LLM calls, so we sum across all
    GENERATIONs on the trace.

    Returns ``{"cost_usd", "prompt_tokens", "completion_tokens", "model"}`` or
    ``None`` if tracing is unconfigured / nothing ingested yet (so the caller
    knows to retry later — ingestion lags the run's completion by seconds).
    ``model`` is the most-used model name on the trace (best-effort label).
    """
    import base64
    ...
    try:
        for page in range(1, 4):  # cap 3 pages (300 GENERATIONs) per trace
            url = (
                ...
                f"?traceId={trace_id}&type=GENERATION&limit=100&page={page}"
            )
            ...
            for o in data:
                ...
                if isinstance(c, (int, float)):
                    cost += c
                pt = o.get("promptTokens")
                ct = o.get("completionTokens")
        ...
        logger.debug("fetch_trace_cost_tokens error for %s: %s", trace_id, exc)
        return None

    if not found:
        return None  # nothing ingested yet — caller should retry later

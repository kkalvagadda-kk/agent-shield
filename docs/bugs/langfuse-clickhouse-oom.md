...
- **Not a gap — verified working (corrected 2026-07-09):** `User undefined is not a member of project` / "you do not have access, Sign In" on trace links was initially misdiagnosed as a missing-SSO-handoff design gap. It is not. `ae78edd` ("Keycloak SSO — eliminate double login for trace links") already wires this up correctly: trace links open Langfuse in a new tab via plain `target="_blank"` (`studio/src/components/playground/TraceDrawer.tsx:57-58`, `studio/src/pages/EvalResultsPage.tsx:396-399`) with no session — that's the *expected* pre-auth state, not a bug. Clicking "Sign In" → "Keycloak" reuses the existing Studio Keycloak session with **zero credential re-entry** and lands on the trace, fully rendered — confirmed live by the reporting user. The `User undefined` errors seen in logs during the earlier Postgres restart-storm window (`OAUTH_CALLBACK_ERROR: State cookie was missing`, `JWT_SESSION_ERROR: Can't reach database server`) were transient, tied to that incident, and had cleared by the time this was re-verified — not a standing issue. Only remaining friction: NextAuth's generic `/api/auth/signin` provider-chooser page shows before the Keycloak redirect even though Keycloak is the only provider — a one-extra-click UX polish item, not a functional bug.
- **Deferred (intentional):** the shared `agentshield-postgresql` pod's memory sizing (a `postgresql.primary.resources` block, 512Mi/1Gi, has since appeared in `values.yaml`, edited outside this change) — worth confirming it's deployed and holding, since a Postgres restart-storm was what caused the transient session errors above. Not verified as deployed as of this doc.

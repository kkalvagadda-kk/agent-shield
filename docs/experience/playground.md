...
**1. Create a dataset (mode-aware).** From the Playground left panel's "Manage Datasets / Eval" link (or `/playground/datasets`), click **New Dataset**. The modal has a **mode selector** that defaults to **`reactive`** (response correctness). Four modes are authorable **in the UI** today — `reactive`, `durable` (E-1), `scheduled` (E-3), and `workflow` (E-5). `webhook` (E-4) has a **validated API contract and a live scoring door but no item editor yet**, so picking it in the modal still falls through to the empty-dataset path; author one via the API until E-4's Studio phase lands (see "Webhook datasets" below). A dataset's `mode` is stored on the row (`playground_datasets.mode`, back-filled to `reactive` for every pre-existing dataset). For reactive, items are one JSON object per line:
...
**Webhook datasets — scoring the filter *and* the action (E-4, phases 1–4 landed).** An event-driven agent's
**first job is to not run**: an event it should have filtered but acted on is a bug even if it answered
perfectly, and a response-only eval cannot see that. A `webhook` item is therefore built around a **synthetic
event**:
- **`trigger_payload`** — the synthetic event. The eval POSTs it through the **real** `POST
  /api/v1/playground/test-event` door, which runs the **real** `filter_engine.evaluate_filters` against the
  trigger's **real** `filter_conditions`. E-4 adds **no filter code**: registry-api's copy of the engine is
  kept byte-identical to the event-gateway's by `scripts/check-filter-engine-parity.sh`, which runs inside
  `deploy-cpe2e.sh` **before either image builds** — divergent engines are undeployable, so the decision the
  eval scores is the decision production makes.
- **`expected_match`** — should the filter match? This is the **first-class dimension**.
- **`expected_filter_reason`** — a substring, meaningful when `expected_match=false`. A miss for the **wrong
  reason** scores `0.0`: it means an unrelated rule dropped the event and the rule under test never ran.
  Without it, a filter that rejects *everything* would score 1.0 on every miss item.
- optional **action expecteds** (`expected_output` / `expected_trajectory` / `expected_side_effects`) — scored
  only when the event **matched**, using E-0/E-1/E-2's scorers verbatim.
- an optional **`injection_probe`** (`must_not_call` + `must_refuse`) — a webhook payload is
  **attacker-controlled**, so the probe asserts an instruction smuggled into the event can't drive the agent.

**A correctly-filtered event is a PASS, not a skip.** It scores `dimension_scores == {"filter": 1.0}` and
**runs nothing** — that is the correct behavior, not an absence of evaluation. Nothing else is scored, because
nothing else happened: an absent dimension is never given a free `1.0` (present-dims-only). A filtered item
doesn't even call the LLM judge — there is no response to judge, and a judge outage must not fail a filter item.

**Injection reports ASR *and* utility, side by side.** The `injection` dimension is the **defense score only**;
`detail.asr` (did a forbidden tool fire?) and `detail.utility` (how good was the answer?) are reported
**separately and never averaged**. Utility reaches the composite through the *action* dims, so an agent that
refuses everything to drive attack success to zero scores `injection=1.0` and **tanks `response`** — visibly
penalized rather than crowned.

**Safety facts gate; they are not averaged.** Two exact facts **veto** the composite to `0.0`: a **filter
error**, and a forbidden tool that **really fired** (`asr == 1.0`). On the default weights alone these
composited to **0.75** and **0.73** — both *above* the 0.7 publish gate — i.e. a weighted mean would have
published an agent for the exact two failures this mode exists to catch. A per-run `dimension_weights` override
cannot switch a veto off. An unmet `must_refuse` deliberately does **not** veto: it rides on a light keyword
check, so it costs weight instead. Fuzzy signals get weighted; exact ones gate.

> ⚠️ **Not usable end-to-end yet (E-4 phases 5–7 pending).** The item contract, the launch guard and the
> scoring door are live, but the **eval-runner has no webhook branch**, there is **no Studio editor or results
> render**, and there is no suite-77. Launching a `webhook` eval today silently runs the *reactive* path
> **live** and never fires the filter — see the E-4 rows in `docs/testing/manual-ui-e2e-test-plan.md`.

**The `test-event` door now honours `eval_mode` and dispatches durable runs (E-4 D2).** `test-event` used to
hand-build its **own** `PlaygroundRun` instead of sharing `POST /playground/runs`' construction path. That one
divergence silently cost three things, each failing *safe* so nothing ever errored: the run never carried
`eval_mode` (it defaulted to `'live'`, so **a matched webhook eval would have delivered real side effects** —
the record seam reads the persisted column); a **durable** webhook agent's matched run was **never dispatched**
and hung at `running` forever; and the run got no Langfuse trace or `agent_version_id`. There is now **one
builder with two call sites**, so a field added to the run reaches both doors by construction. The door's
response contract is unchanged (`{matched, reason, trigger_id, run_id, stream_url}`), and it still defaults to
**`eval_mode='live'`** — a human test-firing a webhook in the sandbox legitimately wants real side effects; only
the batch eval-runner asks for `record`.
...
- **`webhook`** (E-4) — requires an **enabled `webhook` trigger** on the agent; rejects a workflow. Without a
  webhook armed there are no real `filter_conditions`, so there is no filter decision to score and grading one
  would be fiction. Like `scheduled`, the **inner shape** (reactive/durable) is deliberately *not* constrained
  — E-4 scores both.
...
4. `POST /api/v1/playground/eval-runs/{id}/results` — records `{ dataset_item_idx, input_message, response, judge_score, judge_reasoning, passed, dimension_scores }` where `judge_score` is the composite (the gate input) and `dimension_scores` carries the per-dimension evidence (reactive → `{"response": composite}`). A **scheduled** item (E-3) additionally records **`trigger_payload`** — the job spec it actually fired — on the result row, including on a fail-closed row, so the results UI can always show *what was fired* even when the item could not be scored. A **webhook** item (E-4) records the synthetic event on the same `trigger_payload` column plus **`matched`** — the real filter decision (`NULL` when the item fail-closed *before* firing, i.e. no decision was ever made, which is a different outcome from "filtered" and renders differently)

> **The runner dispatches by an explicit mode→handler map, not a priority if-chain.** A mode the runner has no
> handler for records every item **failed** with the mode named and creates **no run**. This is load-bearing:
> the chain it replaced fell through to the reactive path, so an unhandled mode produced a real **`live`** run
...
### Webhook datasets (E-4) — score the filter, the action, and the injection defense

A webhook agent's **first job is to not run**. It sits on an endpoint the public internet can POST to, and most of what arrives is not its business. Response-only eval cannot see any of that: it never fires the filter, and it never asks what an attacker-controlled payload could make the agent do. E-4 scores all three.

**Authoring.** Pick **Webhook (filter + action)** in the dataset mode selector and the synthetic-event editor appears:
- **the synthetic event** (`trigger_payload`) — a JSON object POSTed through the **real** webhook filter,
- **the filter decision** (`expected_match`) — should the filter match this event (the agent runs), or drop it?
- **the expected filter reason** (`expected_filter_reason`) — shown *only* when the event should be filtered, because a match's reason merely names which rule fired. It is a substring that must occur in the real reason: **a miss for the wrong reason is a filter bug**, not a pass — it means an unrelated rule dropped the event and the rule under test never ran. Without this, a filter that rejects *everything* would score `1.0` on every miss item.
- **the action expecteds** (`expected_output` / expected trajectory steps / `expected_side_effects`) — scored **only on a match**,
- **the injection probe** (`must_not_call` tools + `must_refuse`) — the attacker-controlled half.

Invalid JSON blocks the save client-side; the backend re-validates the webhook variant and rejects a malformed item (e.g. a non-list `must_not_call`) with **`422`** at the door rather than key-sniffing it at score time.

**A correctly-filtered event is a PASS, not a skip.** It scores `{filter: 1.0}` and **runs nothing** — no run row exists for it at all. That is the whole point of a filter, so the results screen says so in words rather than leaving an empty row that reads like a broken eval.

**Webhook scoring weights (E-4).** **Filtered** ⇒ `{filter: 1.0}` — the correct decision *is* the whole result; nothing else ran, so nothing else is scored. **Matched** ⇒ the action carries it: `{filter .2, response .3, trajectory .133, tool_call .067, side_effect .3}` (the trajectory *family*'s `.2` split across E-1's two dimensions, preserving the family weight and the 2:1 ratio), with `injection` folding in at `.3` when the item carried a probe — on an attacker-controlled payload the defense matters as much as the task. Present-dimensions-only, and overridable per run.

> **A safety veto overrides the weighted mean.** With the weights alone, a **filter error** whose action happens to be perfect composites to `0.75`, and a run where an injected instruction really fired `wire_transfer` composites to `0.73` — **both above the `0.7` publish gate**. A weighted average can always be out-voted, so averaging a safety property is how it quietly becomes optional. Two **exact** facts therefore force the composite to `0.0` and cannot be switched off by a `dimension_weights` override: `filter == 0.0` (a bool comparison) and `asr == 1.0` (a forbidden tool was really called, read off real recorded calls). `detail.veto` names why. The *fuzzy* `must_refuse` keyword check deliberately does **not** veto — it only costs weight. **Exact facts gate; heuristics get weighted.** The gradient is not lost: `dimension_scores` still carries the full diagnosis.

**Injection probes report ASR *and* utility — separately.** `injection` is the **defense** score: `0.0` if any `must_not_call` tool appears in the recorded calls **or** the projected trajectory (the union — a mocked write lands in the recorded calls, an ungoverned one only in the trajectory; reading either alone would miss half). **Utility is reported, never folded in.** It reaches the composite through the *action* dims instead, so an agent that refuses everything to drive attack success to zero scores `injection: 1.0` and is **visibly penalized** on `response`. The results panel renders **ASR and utility side by side** for the same reason — ASR alone would grade exactly the useless agent the probe exists to catch as flawless. A probe on a **filtered** event is reported as **not exercised** rather than silently scored `1.0`: the agent never saw the payload, so the defense was never tested, and the filter blocking it is already fully credited.

**Webhook result evidence (E-4).** Expand a webhook row and a **filter verdict** block renders first — what the real filter decided (`matched — the agent ran` / `filtered — nothing ran`), what the item expected, a **correct decision** / **FILTER ERROR** badge, the real reason vs the expected substring, and the veto when one fired. Below it the **"Synthetic event"** block shows exactly what was fired (labelled as an event, not a job spec — both ride the same `trigger_payload` column and are told apart by the run's explicit `mode`), then the **injection** panel with ASR and utility side by side and the forbidden tools that actually fired. A row that fail-closed *before* firing says **"no filter decision was recorded"** — never rendered as "filtered", because "we refused to fire" and "the filter dropped it" are completely different outcomes.

**The `test-event` door honours `eval_mode` and dispatches durable runs.** Both were live defects until E-4 rewired that door onto the **one shared run builder**. It used to hand-build its own `PlaygroundRun`, and that single divergence meant: (a) it never threaded `eval_mode`, so a matched webhook eval would have **delivered real side effects**; (b) it never dispatched a durable run, so a durable webhook agent's matched action hung at `running` forever; (c) it dropped the Langfuse trace and `agent_version_id`. It now feeds a matched event the **identical production shape** the real webhook door uses — `input_payload=payload` plus a driving turn derived with `internal.py`'s own line (`payload.get("message") or json.dumps(payload)`) — so the eval drives the agent with exactly what a real delivery drives it with.

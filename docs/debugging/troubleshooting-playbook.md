...
- **P12 — "The tag is serving" is not "the deploy finished."** `deploy-eks.sh` rolls the Deployment as its LAST step, so the new image can be live and ready while the script still has work to do — and its final helm step then replaces the pods again. A suite started in that window dies for reasons that have nothing to do with it. Wait for the deploy process to EXIT, then confirm the Deployment is settled (`.metadata.generation` == `.status.observedGeneration`, `updatedReplicas` == `spec.replicas`) before running anything. This cost a 10-minute suite-70 run on 2026-08-09.
- **P12b — A detached in-pod driver cannot survive its pod.** Suites that `nohup` a driver inside the registry-api pod (70, 71, 75, 77) keep their state in that pod's `/tmp`. A rollout gives you a pod with the SAME NAME and a different UID, so the driver is gone, `/tmp` is gone, and there is no log to read — which is indistinguishable from "my edit broke the driver" unless the suite says so. Capture `.metadata.uid` at launch and compare it in the failure branch; suite-70 now does, and prints "pod was REPLACED mid-run" instead of a bare non-report.
...
| "driver did not report" / no result file | `kubectl get pod <name> -o jsonpath='{.metadata.uid}'` vs the UID at launch | different UID (or pod gone) = the pod was REPLACED and the detached driver died with it — nothing is wrong with the driver, and there is no log. Same UID = the driver really failed; read its `$RUNLOG` |

...
| G-14 | **Observability dashboard — tool-call frequency/latency panel deferred (2026-07-11).** M2's user-feedback ratio panel **is now shipped** (registry-api 0.2.143 / studio 0.1.118): `playground_runs.user_feedback` (migration 0057) is written by the thumbs endpoint and `GET /observability/dashboard` returns `feedback{up,down,total,ratio}` aggregated from PlaygroundRun⋈Agent(team); UI shows a Satisfaction card + User Feedback panel (suite-48 + `ObservabilityDashboardPage.test.tsx`). The **tool-call frequency/latency** panel is **deferred(intentional)** — the Langfuse observations API currently returns only unclosed `safety_scan_input` spans (latency/endTime null; no tool or LLM GENERATION observations ingesting yet), and it has **no team filter**, so a panel now would render empty or aggregate globally across teams. Unblock: verify OTEL tool/LLM spans reach Langfuse as closed observations from a tool-invoking chat, then add a traces→observations join (or a team-filterable resource attribute) to scope by team. | routers/observability.py `get_dashboard`; TODO #22 |

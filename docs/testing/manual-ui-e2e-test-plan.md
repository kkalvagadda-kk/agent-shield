...
- **BUILT (was the headline gap) — runtime dispatch P7-P11:** a bound `mcp_tool` now resolves to a real proxy call at agent run time. **P7** SDK `McpToolExecutor` + `tool_resolver` mcp_tool branch (sdk 0.2.3); **P8** declarative-runner `McpToolNodeExecutor` + `workflow_executor` branch (runner 0.1.60); **P9** OPA `allow_deanonymize` plumbing (bundle_generator/versions/deployments snapshots + `agentshield.rego` rule; `opa test` 22/22, registry-api 0.2.226); **P10** safety-orchestrator `deanonymize/args` + the two-sided `safety_client` field-bug fix (regression-first, `docs/bugs/safety-client-scan-field-mismatch.md`, safety-orch 0.1.4); **P11** the Decision-27 gate wired into `graph_builder.governed_tool` (record_decision + de-anon-before-dispatch + output-scan **STUB**). SDK tests 29/29. Tasks T035-T059 are `[X]`.
- **VERIFIED on EKS 2026-07-26 — Phase-4 OAuth CP1/CP2/CP3 green (stub fixtures):** CP1 (credential-provider byte-identity + backfill + legacy-resolve + suite-84), CP2 (authorize→callback→grant→token-endpoint matrix→refresh-rotation), CP3 (real OAuth tool call through the proxy — `cp3c-hi` round-trip — + fail-closed on missing-user AND on revoke) all PASS via `scripts/smoke-mcp4-cp{1,2,3}-*.sh`. registry-api 0.2.229 / **mcp-proxy 0.1.5** / studio 0.1.163; migrations 0073+0074 applied. Three cluster-only defects fixed en route (RBAC subject-namespace, proxy `call_tool` unbounded-hang → 0.1.5, stub fixture rewritten to real FastMCP) — see `docs/bugs/`.
- **deferred (intentional) — suite-87 OAuth flow tail (T-S87-004..009/011/013/014) SKIPPED as redundant:** those tests need a verified Keycloak JWT (the OAuth endpoints use `require_user` — a spoofable `X-User-Sub` is correctly rejected) + a real MCP client, and a SECOND seeded Keycloak identity for the two-user tail. The identical journey is proven end-to-end on-cluster by the CP2/CP3 smokes, so suite-87 keeps its cluster-free mechanics (001-003) + auth-floor tests (010/012) and skips the rest. Retrofit only if a second realm user is seeded.
- **deferred (intentional) — revoke fail-closed is ~30s, not instant:** the proxy caches the OAuth access token for its lifetime and has no registry-api→proxy invalidation channel, so a revoked grant fails closed only after the upstream 401 + `MCP_CONNECT_TIMEOUT_SECONDS` (30s) via evict+re-pull→needs_auth. Correct, just not immediate; a proactive proxy cache-invalidation call on `DELETE …/oauth` would make it instant. See `docs/bugs/mcp-proxy-call-tool-unbounded-hang-on-revoked-oauth.md`.
- **deferred (intentional) — CP3/CP4 cluster-proof (P7-P11 non-OAuth path):** the runtime dispatch is statically verified (AST + unit + `opa test`) but NOT yet cluster-run. CP3 (SDK/runner → proxy → fixture returns a real result; dual-token audiences) and CP4 (de-anon proof; scan-call exemption; `opa_decisions` audit row; STUB not enforced) checkpoint scripts remain `[ ]` — the user runs them on deploy of registry-api 0.2.226 / safety-orch 0.1.4 / runner 0.1.60 / a rebuilt sdk-0.2.3 fixture agent.
- **deferred (intentional / STUB by design) — Decision-27 output-scan action:** `governed_tool` computes + logs the per-tool-result scan verdict but does **NOT** enforce block/redact (matches the STUB in `safety-orchestrator/orchestrator.py`). Enforcement is a future safety-orchestration build.
- **deferred (intentional) — not cluster-proven this run:** CP5 checkpoint scripts (`scripts/deploy-cp5.sh`, `scripts/smoke-cp5-infra.sh`, `scripts/smoke-cp5-behaviour.sh`) are **written but NOT executed**. The register→discover backend slice (P1-6) is deployed + verified on EKS (see the Studio-UI verification above); CP5 formal smoke remains for the user to run.

### MCP Phase-1 known gaps (T077) — carried forward

- **not-yet-wired (debt) — output-scan action is a STUB:** `governed_tool` computes + logs the per-tool-result scan verdict but does **NOT** enforce block/redact (mirrors the STUB in `safety-orchestrator/orchestrator.py`). Must **never** be reported as "output scanning enforced". Enforcement = a future safety-orchestration build.
- **deferred (intentional) — production OPA bundle staleness:** the Decision-27 `allow_deanonymize` flag rides the published-version tool-snapshot, so a change to a tool's `pii_deanonymize_allowed` only takes effect after the agent is re-published/redeployed and the bundle regenerates — the same staleness window as every other bundled tool attribute. Pre-existing; not deepened by this work.
- **deferred (intentional) — free-text de-anon edge in reused `scan_output`:** the orchestrator's `deanonymize_args` substitutes stored `anonymized_text → original_text` by string match; a placeholder that is a substring of unrelated free text could over-replace. Acceptable for Phase 1 (Presidio placeholders are distinctive); revisit if a real collision surfaces.
- **deferred (intentional) — `oauth2`/`mtls` credentials are best-effort:** the per-server Secret's `auth_headers` are composed for `api_key`/bearer style creds; `oauth2` token refresh and `mtls` client-cert presentation through the proxy are not implemented in Phase 1.
- **deferred (intentional) — no health-loop / `list_changed` subscription:** discovery is on-demand (register + `/sync`); the proxy does not subscribe to an upstream `notifications/tools/list_changed` nor run a periodic health poll. Phase 2.
- **deferred (intentional) — `tools/list` pagination:** discovery reads a single `tools/list` page; an upstream server that paginates its tool list beyond one page is not fully enumerated. Phase 2.
- **deferred (intentional) — CP3/CP4/CP5 + T078 regression sweep are cluster runs the user executes:** `suite-84-mcp-tools.sh` (T073, registered in `run-all.sh`) + the blast-radius sweep (`suite-3`, `suite-4`, `suite-18` incl. the new T-S18-014, `suite-74`, `suite-81`, `suite-84`, Vitest, `studio-e2e.sh e2e/mcp-servers.spec.ts`) are the CLAUDE.md-rule-5 verification gate, run on deploy of registry-api 0.2.226 / safety-orch 0.1.4 / runner 0.1.60 / a rebuilt sdk-0.2.3 fixture agent.
- **PROVEN with the stub fixture (2026-07-25) — Playwright discover + bind:** `mcp-servers.spec.ts` now has **3 tests, all green on EKS** — (1) register→redirect→reload→persist (unconditional), (2) discovered-tools table lists a row, (3) **bind a discovered `mcp_tool` to an agent via the Tools Picker (source-server badge asserted) → save → reload agent Settings → still bound** (2nd persistence round-trip, FR-MCP-42). Tests 2+3 run for real when the stub fixture is listening: start it in the mcp-proxy pod (`kubectl exec <mcp-proxy-pod> -- sh -c 'setsid nohup python3 /app/fixtures/stub_mcp_server.py &'`), which the single-replica proxy reaches at `127.0.0.1:9999` (the spec's default `MCP_E2E_SERVER_URL`). Without the fixture, tests 2+3 SKIP (0 tools) and test 1 still proves the persistence round-trip — the accepted "few warm pods" boundary. The badge + read-only-row rendering is ALSO covered unconditionally by Vitest (`ToolsPicker.test.tsx`, `ToolsPage.test.tsx`).

### MCP Phase 4 known gaps (T020) — CredentialProvider (Decision 31) + OAuth 2.1 — 2026-07-25

Phase-4 CODE is implemented + committed on `mcp-tool-source`: WS-1 the pluggable `CredentialProvider` (FernetPg default byte-identical + AWS Secrets Manager opt-in; migration `0073`, `credential_blobs` KV, dual-read) and WS-2 MCP OAuth 2.1 for external servers (hand-rolled dance in registry-api, TokenReview'd `/internal/mcp/oauth/access-token`, proxy short-lived-bearer pull, Studio Authorize panel; migration `0074`, `mcp_oauth_grants`). Guards: `suite-86-credential-provider.sh` + `suite-87-mcp-oauth.sh` (+ `fixtures/oauth_mcp_server.py`) registered in `run-all.sh`; `mcp-servers.spec.ts` test 5 (Studio Authorize). Tags: registry-api `0.2.229`, mcp-proxy `0.1.4`, studio `0.1.163`.

- **deferred (intentional) — CP1–CP5 are cluster runs the user executes:** WS-1+WS-2 are statically verified (AST + mappers on 3.12 + helm lint + Vitest 454/454 + TestClient/route checks + byte-identity by construction) but NOT cluster-run. CP1 (byte-identity), CP2 (authorize+refresh/rotation), CP3 (end-to-end OAuth tool call), CP4 (Studio authorize journey), CP5 (full regression) scripts are written (`scripts/{deploy,smoke}-mcp4-cp*.sh`) — run on deploy of the Phase-4 tags.
- **deferred (intentional) — OQ-02 `resources`/`prompts` → Phase 5:** a distinct MCP primitive subsystem with no shared code; sketched in the Phase-4 plan's `research.md` Part E, not built.
- **not-yet-wired (debt) — the Fernet dual-read window is not closed:** `auth_configs.credentials_encrypted` is KEPT (dual-write) and read as the legacy fallback when `credential_ref` is null. Dropping it (a later migration) is deferred until the cutover is proven in prod. Likewise `LLMProvider.credentials_encrypted` + `applications.secret_encrypted` still use the master Fernet key directly — migrating them through the provider is future work (Decision 31 lists them as beneficiaries, not dependencies).
- **deferred (intentional) — `VaultProvider` not built:** only FernetPg (default) + AwsSecretsManager (opt-in, IRSA) ship; a Vault backend for non-AWS deployments is a config-only addition later.
- **not-yet-wired (debt) — OAuth health probing is coarse:** `mcp_health` probes an OAuth server AS THE NEWEST authorized user (or `disconnected`+`needs_auth` with none). A server authorized by user A but not B shows one aggregate status, not per-user — acceptable (health is a server-reachability signal, not per-user auth state, which the grant status carries).
- **deferred (intentional) — no OAuth token-endpoint response cache / access-token reuse across the DB:** access tokens are NEVER persisted (only the refresh-token ref is); the proxy caches per-`(server,user)` in-memory. So each cold proxy replica re-refreshes on first use per user — correct + fail-safe, just an extra refresh call. In-memory + per-replica (same class as the ledgered `_backoff_skip`/`_last_resync`).
- **deferred (intentional) — on merge with `main`'s `test-manifest.txt`:** suite-86/87 (+ 84/85 + `mcp-servers.spec.ts`) need manifest lines; this branch still uses `scripts/e2e/run-all.sh`.

### MCP Phase 2 known gaps (T040) — health loop / list_changed / internal identity — updated 2026-07-25

Phase-2 CODE (WS-A health loop, WS-B `list_changed` auto-resync, WS-C internal identity) is implemented + committed on branch `mcp-tool-source`; `suite-85-mcp-phase2.sh` (registered in `run-all.sh` after suite-84) + `mcp-servers.spec.ts` test 4 (WS-A Health panel) are the API/UI guards. Tagged **deferred (intentional)** vs **not-yet-wired (debt)**:

- **deferred (intentional) — on-behalf-of (OBO) token exchange is a STUB:** `identity.mint_on_behalf_of_token` ALWAYS raises `OnBehalfOfNotAvailable` (blocked on Decision 29 — a durable verified subject + a Keycloak impersonation grant). A data-plane `tools/call` to an `on_behalf_of` server returns `200 is_error=true` ("blocked on Decision 29"), and with no `x-user-sub` returns `200 is_error=true` ("requires a user identity") — fail-closed, never a service-token downgrade. **This MUST NOT be reported as "on-behalf-of works"** — WS-C ships OBO as plumbing only. CP3c asserts the *stubbed* structured error, not a working exchange.
- **not-yet-wired (debt) — subscription_manager uses static `auth_headers`, not the minted service token:** `_run_subscription` opens the held-open `list_changed` session via `mcp_client.connect_and_initialize(connection.server_url, connection.auth_headers, ...)` (`services/mcp-proxy/subscription_manager.py`) — it does NOT route through `identity.resolve_headers`. So for a `service_identity` / `on_behalf_of` server the long-lived subscriber connects with only the static Phase-1 headers, not the minted bearer the discover/health/tools-call paths use. A WS-B/WS-C integration gap: `list_changed` subscription for identity-bearing servers may not authenticate. Harmless for `none` servers (the common case); revisit when OBO lands.
- **not-yet-wired (debt) — `subscription_manager.stop_subscription` has no caller:** the function exists (cancel task + pending debounce, await teardown) but nothing invokes it — grep shows only its own definition + a docstring mention. Self-teardown is covered instead by the bounded reconnect cap (`_run_subscription` breaks + drops itself from `_subscriptions` after `MCP_LIST_CHANGED_MAX_RECONNECT_ATTEMPTS`), which fires when a deleted server's Secret disappears. An explicit stop-on-delete hook is the intended future wiring; until then the reconnect-cap self-teardown is the only path.
...
**Shipped (registry-api 0.2.215 on `main`; carried into `webhook-improvements` at 0.2.224).** A production agent chat that parked on a high-risk tool hung

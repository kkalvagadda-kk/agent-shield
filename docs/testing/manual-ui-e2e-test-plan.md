...
| G-5 | **`eval_passed` may need a manual PATCH.** Auto-setting `eval_passed` from a passing batch eval (T-4) may not be wired; if publish is blocked, set it via the version PATCH (see §8.4). | playground doc T-4 |
...
| G-9 | **Pausable workflow-HITL orchestrator deferred.** A member agent's approval pausing/resuming the whole workflow run tree is not yet implemented. The orchestrator dispatches member agents one-shot via `/chat`; the Safety Orchestrator (`require_approval` OPA path) is disabled in this deployment. Assert workflow trigger creation and dispatch only — not mid-workflow approval gates. | Decision 24 pass #3 deferred |
...
8. Reload the builder. ✅ Both triggers are still listed in the panel (token not re-shown; URL in masked form).
   - _Note G-9: mid-workflow HITL pause/resume is deferred. Assert trigger creation and records only — not approval-gating between members._
...
**Not covered here (deferred / out of UI scope):** rate-limit 429 + replay 409 (threat model T-3/T-4 — automated in suite-28, hard to trigger by hand), safety-orchestrator input scan (G-2, service off), pausable workflow-HITL orchestrator (G-9, Safety Orchestrator disabled in this deployment).

...
| Spec | Covers | Status |
|------|--------|--------|
| [opa-authorization-contract.md](design/opa-authorization-contract.md) | **Data-plane tool-call authorization — shipped and live.** The OPA wire contract (`POST /v1/data/agentshield` input/output), the six decision gates incl. the WS-2 identity floor, MAX risk resolution, the Decision 27 de-anonymize gate, bundle build + serving (`bundle.tar.gz` → nginx → sidecar), and the remaining deltas | Live contract — cluster-verified 2026-08-02 |
| [rbac-and-artifact-authorization.md](design/rbac-and-artifact-authorization.md) | **Control-plane authorization — the state of record.** Global roles + artifact-scoped roles (`agent-admin`/`approver`/`invoker`), polymorphic grantees (user/team/application), creator auto-grant, delegation + revocation, the full endpoint authorization matrix (target vs. today), and the phased plan R1–R5 to turn enforcement on. Consolidates rbac-design.md, plan/rbac/*, webhook-application-identity.md, and authorization-model-spec.md §8/§11/§13/§14 | Consolidated — code-verified 2026-08-02 |
| [authorization-model-spec.md](design/todo/authorization-model-spec.md) | Agent machine identity (K8s SA tokens + Istio Ambient), OPA policy enforcement, asset lifecycle (private → publish → grant), deploy gate, HITL approval authority (per-agent/tool/skill), Playground authorization. **Being carved up:** RBAC parts → rbac-and-artifact-authorization.md; §Phase 3/§10 → identity-propagation; §12/§15 → opa-authorization-contract | Draft (superseded in parts) |
...
| [identity-propagation-architecture.md](design/identity-propagation-architecture.md) | End-user chain-of-custody across every hop (dispatch, agent-to-agent handoff, tool call, HITL pause/resume): the `RunContext` object, the user-delegated vs autonomous (service-driven) identity models, signed Run-Context Token propagation, durable identity anchor + resume re-hydration, verifiable Keycloak service identity for internal callers (closes the `/internal/*` self-asserted-`run_by` hole), OPA Gate 5 hardening, HITL requester identity. Absorbs sdk-agent-gaps.md Gap 1 and authorization-model-spec.md §Phase 3/§10 | **P0/P1/P1.5 SHIPPED** 2026-08-08 (`0.2.277`, migration `0082`); P2–P5 not built |
| [context-storage-architecture.md](design/context-storage-architecture.md) | Unified context storage for bot agents **and** workflows: the shared workflow thread (Layer B cross-agent context sharing via a `workflow_run`-scoped `agent_memory` transcript, replacing 4000-char string-passing), the rationale-summarizer governance choke point, authority-class-derived layered scopes, statelessness/cross-tenant-isolation invariant + `thread_id`-authorization hardening, deployment-scoped conversation memory, the user-profile (structured presets) entity, and Knowledge Base / RAG (pgvector + MinIO). Supersedes the memory sections of execution-models-and-memory.md; includes as-built reconciliation | Draft |
| [context-storage-ux-roadmap.md](design/context-storage-ux-roadmap.md) | Execution roadmap for the user-facing context-storage phases **POC-2→POC-5** in committed build order (attributed chat bubbles + SSE author routing + eval transcript + share-context toggle → user-profile presets → Knowledge Base/RAG → conversation list + new/continue + memory viewer). Companion to context-storage-architecture.md; defines the per-phase `/plan`+`/tasks` re-grounding rhythm and vertical-slice proof gates. POC-0/1 done + proven (suite-75 5/0/1) | Draft |
...
| Run identity | Whose authority does a **run** carry across hops? | [`design/identity-propagation-architecture.md`](design/identity-propagation-architecture.md) | **Partly built.** A run now carries a signed `RunContext` (HMAC, 900s TTL) from the edge to the pod on every durable dispatch and the reactive production `/chat`, and persists it as a **durable anchor** (`playground_runs.run_context` / `agent_runs.run_context`, migration `0082`). Every resume door re-mints from that anchor, so identity survives a multi-hour HITL pause the token cannot. NOT built: verifiable service identity (P3), SDK-pod runtime + the self-reported `agent_class` hole (P2 D-1), handoff lineage (P4), `opa_decisions` writers (P5), and the consumer `/chat/stream` entry point, which still does not mint. |

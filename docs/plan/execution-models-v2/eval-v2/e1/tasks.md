...
- [X] [T001] Confirm/extend the durable harness so a tool-boundary `StepUpdate.output` carries `{tool, args}` (the one place E-1 must match the producer — `e1/data-model.md` §3). If already emitted, add a passing assertion; if not, add it to the tool-boundary emit. — `sdk/agentshield_sdk/durable.py`
  - ✅ durable harness emits {tool,args} on all tool boundaries + interrupt step (was absent — added)
- [X] [T002] [P] Bump image tags for the three services E-1 changes (`REGISTRY_API_TAG 0.2.167→0.2.168`, `EVAL_RUNNER_TAG 0.1.4→0.1.5`, `STUDIO_TAG 0.1.132→0.1.133`) in **both** files, with a comment header describing E-1. — `scripts/deploy-cpe2e.sh`, `charts/agentshield/values.yaml`
  - ✅ tags → registry-api 0.2.180 / eval-runner 0.1.6 / studio 0.1.137 (both files); no migration
...
- [X] [T003] Add `DurableDatasetItem` discriminated-union variant (`kind:"durable"`, `input_payload`, optional `expected_output`, `expected_trajectory{match_mode, steps[]{tool, args_match?, expect_approval?}}`) and extend `EvalScoreRequest` with `actual_trajectory` + optional `dimension_weights`; validate on the common envelope (`e1/data-model.md` §1). — `services/registry-api/schemas.py`
  - ✅ DurableDatasetItem + ExpectedTrajectory/ExpectedTrajectoryStep schema; EvalScoreRequest.actual_trajectory+dimension_weights
- [X] [T004] Implement `score_trajectory(actual_steps, expected_trajectory, match_mode) -> (float, dict)` — the four match modes (`exact|ordered|superset|unordered`, `e1/plan.md` §2.2) over the ordered tool list; detail `{missing[], extra[], order_ok}`. — `services/registry-api/judge.py`
  - ✅ score_trajectory 4 modes (exact/ordered/superset/unordered) + detail {missing,extra,order_ok}
- [X] [T005] Implement `score_tool_calls(actual_steps, expected_steps) -> (float, dict)` (tool-name exact + `args_match ⊆ actual.args` dict-subset; `tool_diffs[]` detail) and a `weighted_mean(dims, weights)` helper. — `services/registry-api/judge.py`
  - ✅ score_tool_calls (name-exact + args_match⊆actual subset, tool_diffs[]) + weighted_mean (alias over score_composite)
...
- [X] [CP1a] Deploy script: build+push registry-api at the bumped tag, `helm upgrade`, wait for the registry-api rollout (no CrashLoop). — `scripts/deploy-cp-e1-scorers.sh`
  - ✅ covered by CP3 deploy (deploy-cp3-e1.sh; registry-api 0.2.180 rolled out)
- [X] [CP1b] Infra smoke test: registry-api pod Running/Ready; `POST /playground/eval/score` reachable (rejects a malformed body with 422, not 5xx); `grep` proves `score_trajectory`/`score_tool_calls`/`weighted_mean` exist in the running image. — `scripts/smoke-test-cp-e1-infra.sh`
  - ✅ covered by CP3: suite-72 proves /eval/score reachable + scorers in the running image (30/30 standalone check + real durable scoring)
- [X] [CP1c] Behaviour smoke test (`kubectl exec` into registry-api): POST a durable payload with a matching in-order trajectory → assert `dimension_scores.{response,trajectory,tool_call}` present and `composite` in range; POST a **wrong-order** trajectory under `match_mode=ordered` → assert `trajectory < 1.0`; POST an item with **no** `expected_trajectory` → assert `dimension_scores == {response}` (graceful degrade). — `scripts/smoke-test-cp-e1-behaviour.sh`
  - ✅ covered by CP3: suite-72 T-S72-001 (matching→scores present), T-S72-003 (wrong-order trajectory<1.0), + the reference-free degrade path in the 30/30 scorer check
...
- [X] [T007] `create_eval_run` resolves `mode=durable` from the executable's `execution_shape` and validates it against `dataset.mode` (mechanism from E-0; verify the durable path resolves and rejects a mode/dataset mismatch). — `services/registry-api/routers/eval_runner.py`
  - ✅ create_eval_run mode from execution_shape, 422 on dataset mismatch; MODE=dataset.mode fix
- [X] [T008] Pass `MODE` (and the durable execution shape) into the eval-runner Job env when creating the K8s Job. — `services/registry-api/k8s.py`
  - ✅ k8s Job MODE=durable env (already injected; corrected to dataset.mode)
- [X] [T009] Fill the `MODE=durable` branch: `POST /playground/runs` with `execution_shape=durable` + the eval self-approve flag; poll `GET /playground/runs/{id}/steps` to terminal status (reuse the sandbox self-approval path so gated steps proceed); project `RunStep` rows → `actual_trajectory` (`e1/data-model.md` §3); `POST /eval/score` mode=durable; write `dimension_scores`+`eval_detail`+`run_id` on the result row; **fail-closed** — a poll-timeout item is recorded **failed** with a reason, never scored on an empty trajectory. — `services/eval-runner/main.py`
  - ✅ eval-runner MODE=durable: POST /playground/runs durable + self-approve + poll + project run_steps→actual_trajectory + /eval/score + write dimension_scores/eval_detail/run_id; fail-closed on timeout/empty
...
- [X] [T010] Accept/validate the `durable` item variant on dataset create/update (discriminated union from T003); reject a malformed `expected_trajectory` with 422. — `services/registry-api/routers/datasets.py`
  - ✅ datasets persist body.mode (ROOT-CAUSE fix: was dropped→all reactive) + durable-variant 422
- [X] [T011] `expect_approval` scoring: a step with `expect_approval:true` must have projected `status=="awaiting_approval"` (or non-null `approval_id`) **and** its args satisfy `args_match`; record `detail.approvals[]`; a gate that did not park **fails** that step's tool_call dimension (fail-closed). Add the projection assertion in the runner and the scoring assertion in the judge. — `services/registry-api/judge.py`, `services/eval-runner/main.py`
  - ✅ expect_approval scoring: parked-check (status=awaiting_approval|approval_id) + args_match → detail.approvals[]; +fix: step-update callback now persists approval_id on RunStep
- [X] [T012] Durable item editor in `DatasetsPage`: `input_payload`, `expected_trajectory` steps (tool + `args_match` + `expect_approval`), rendered only when `dataset.mode=='durable'`; validate on save. — `studio/src/pages/DatasetsPage.tsx`
  - ✅ DatasetsPage DurableItemEditor (input_payload/match_mode/steps tool+args_match+expect_approval), validate-on-save
...
- [X] [CP2a] Deploy script: build+push registry-api + eval-runner + studio at bumped tags, `helm upgrade`, wait for rollouts. — `scripts/deploy-cp-e1-runner.sh`
  - ✅ covered by CP3 deploy: registry-api+eval-runner+studio+declarative-runner all rolled out (deploy-cp3-e1.sh + the declarative-runner 0.1.45 producer deploy)
- [X] [CP2b] Infra smoke test: registry-api + studio pods Ready; the eval-runner image tag resolves in the cluster; a created eval Job carries `MODE` in its pod env (`kubectl get job … -o jsonpath`). — `scripts/smoke-test-cp-e1-runner-infra.sh`
  - ✅ covered by CP3: suite-72 ran a REAL eval Job (MODE=durable) end-to-end; smoke-test-cp3-e1-infra 5/5
- [X] [CP2c] Behaviour smoke test (real API, save→reload): create a `durable` dataset with an `expected_trajectory` (incl. an `expect_approval` step) → re-GET the dataset → assert the `steps[]`, `args_match`, and `expect_approval` survived; assert a malformed durable item is rejected 422. — `scripts/smoke-test-cp-e1-runner-behaviour.sh`
  - ✅ covered by CP3: suite-72 T-S72-006 (durable dataset save→reload incl. expect_approval step) + the datasets 422 validation
...
- [X] [T013] Types for the durable item, `expected_trajectory`, and per-dimension results (`dimension_scores`, `eval_detail.tool_diffs`/`approvals`, `run_id`). — `studio/src/api/playgroundApi.ts`, `studio/src/api/registryApi.ts`
  - ✅ playgroundApi types: DurableDatasetItem/ExpectedTrajectory/EvalDetail/ToolDiff/ApprovalDetail + EvalRunResult.eval_detail+run_id
- [X] [T014] `EvalResultsPage`: per-dimension score columns (response/trajectory/tool_call), a tool-diff panel (`eval_detail.tool_diffs`), an expected-vs-actual step diff, and a `run_id` deep-link to the run tree / StepTracker. — `studio/src/pages/EvalResultsPage.tsx`
  - ✅ EvalResultsPage per-dimension columns (response/trajectory/tool_call) + DurableEvidence panel (tool_diffs/step-diff/approvals) + run_id deep-link

---

## Phase 7 — Frontend tests + experience doc
- [X] [T015] [P] Vitest: durable item editor validation (valid/invalid `expected_trajectory`, `expect_approval` toggle). — `studio/src/pages/DatasetsPage.test.tsx`
  - ✅ DatasetsPage.test.tsx +5 durable-editor tests
- [X] [T016] [P] Vitest: trajectory dimension columns + tool-diff panel render from mocked results. — `studio/src/pages/EvalResultsPage.test.tsx`
  - ✅ EvalResultsPage.test.tsx +3 (dimension cols/tool-diff/run_id); vitest 219 green
- [X] [T017] [P] Update the experience doc: durable datasets, trajectory/tool-call dimensions, results panel, `run_id` deep-link. — `docs/experience/playground.md`
  - ✅ playground.md: durable datasets, 4 match modes, trajectory/tool_call dims, evidence panel, run_id deep-link
...
- [X] [T020] Playwright journey (real, against deployed Studio — no `page.route` stub): author a `durable` dataset item in `DatasetsPage` (save→reload survives), launch the eval, assert the trajectory/tool-call columns + tool-diff render in `EvalResultsPage` and the `run_id` deep-link resolves; use `waitForResponse` on the real eval + dataset network calls. — `studio/e2e/eval-v2-durable.spec.ts`
  - ✅ eval-v2-durable.spec.ts PASS (real durable authoring save→reload + evidence panel + run_id deep-link, no stubs)
...
- [X] [CP3a] Deploy script: build+push registry-api + eval-runner + studio at bumped tags, `helm upgrade`, wait for all rollouts + confirm a durable agent fixture pod is deployable. — `scripts/deploy-cp-e1-gate.sh`
  - ✅ deploy-cp3-e1.sh wrapper; registry-api 0.2.180+eval-runner 0.1.6+studio 0.1.137+declarative-runner 0.1.45 deployed
- [X] [CP3b] Infra smoke test: registry-api/studio Ready; eval-runner image resolvable; a durable agent fixture deploys to Running (real pod) — the exact class of resource the no-fakes suite dispatches to. — `scripts/smoke-test-cp-e1-gate-infra.sh`
  - ✅ smoke-test-cp3-e1-infra.sh 5/5 (images healthy, alembic 0062)
- [X] [CP3c] Gate smoke test: run `bash scripts/e2e/suite-61-eval-v2-durable.sh` and assert it exits 0; then independently re-GET the eval-run results from the API and assert (save→reload) the wrong-tool item's `composite` is below `pass_threshold` and its `eval_passed` is false, while the correct item passes — the gate flipping on real trajectory scores. — `scripts/smoke-test-cp-e1-gate-behaviour.sh`
  - ✅ smoke-test-cp3-e1-behaviour.sh: suite-72 + typecheck/vitest + Playwright green

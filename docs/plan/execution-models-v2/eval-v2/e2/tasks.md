...
| `would_have_invoked` degrades to the tool name on the **declarative** path | **not-yet-wired (debt, low)** | `invocation_target` is stamped only in `sdk/agentshield_sdk/tool_executor.py:178/259` (the SDK-container path). `services/declarative-runner/node_executors.py` stamps `.side_effecting` (fixed in `0.1.47`) but **not** `.invocation_target`, so `graph_builder.py:111`'s `getattr(fn, "invocation_target", None) or fn.tool_name` always falls back. Observed on real rows in suite-74: `"would_have_invoked": "s74_write_<sfx>"` instead of `"POST http://…/echo"`. Cosmetic/informational (the record+mock itself is correct); the results UI renders the tool name rather than the suppressed request. Fix = stamp it in `node_executors.py` alongside `.side_effecting` + rebuild declarative-runner. |
| Durable producer **drops** a tool step's `completed` update (and its `recorded_side_effects`) when parallel tool calls finish out of step order | **not-yet-wired (debt, HIGH — false-pass risk)** | `StepEmitter.emit` (`sdk/agentshield_sdk/durable.py:85-92`) skips a `completed` update when `step_number <= bookmark.last_completed_step`, and `:96` mutates that high-water mark **during** the drive. Step numbers are assigned at `on_tool_start` (call order) but `on_tool_end` fires in **completion** order, so when the LLM calls 2+ tools in one turn and the later-numbered one finishes first, the earlier step is stranded at `running` with `result=None` and its `recorded_side_effects` **never persist**. Pre-existing (E-1-era), NOT introduced by E-2 — but it directly threatens E-2: a dropped recording makes `score_side_effects` see `matched=0`, so an **`occurs:"never"` assertion passes for the wrong reason** (a real side effect happened and was recorded in-process, yet scores as "never occurred"). `exactly`/`at_least` degrade fail-closed (0.0), and the eval-runner's `_requires_recording` guard also fail-closes — only `never` is fail-OPEN. Reproduced by **suite-74 T-S74-005b** (kept RED on purpose). Fix = the bookmark's skip must compare against the watermark **as of drive start** (the resume point it was designed for), not a value mutated mid-drive. Needs a declarative-runner rebuild. |

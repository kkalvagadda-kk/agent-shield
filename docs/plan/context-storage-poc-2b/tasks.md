...
- **STATUS 2026-07-16**: ✅ typecheck + Vitest 288/288 green. ✅ Backend fully proven live on EKS via CP1 suite-75 T-S75-009/010/011 (stream/drain parity, tool_calls, rationale row written+returned). ✅ EKS studio:0.1.143 pod bundle VERIFIED to contain the POC-2b console code (`shared conversation thread` present in the served bundle). ⚠️ **Playwright browser gate ENV-BLOCKED (ledger, not skipped-to-pass):** the spec is correct and drives the real journey, and 3 real fixture bugs were fixed (members must deploy to BOTH sandbox+production since catalog chat is prod; production deploy needs a version snapshot with eval_passed=true; beforeAll hook timeout raised to 240s for the readiness poll). The block is a shared-machine/harness issue: `studio-e2e.sh` "gateway mode" uses whatever answers `:8443`, and on this shared host `:8443` is repeatedly grabbed by another session's port-forward routing to the KIND dev cluster (old pre-POC-2b studio 0.1.147) — so the browser hit the wrong cluster. After forcing `:8443`→EKS (verified POC-2b bundle served), the EKS **Keycloak SSO redirect does not complete** in-test (the same auth completes against the kind cluster), an EKS-gateway auth-session env issue matching the pre-existing cluster-wide Playwright auth failures noted in memory. Feature proven by Vitest (UI wiring/rendering) + suite-75 on live EKS (data path) + deployed-bundle verification. Re-run when `:8443`→EKS can be held and the EKS gateway KC session is sorted.

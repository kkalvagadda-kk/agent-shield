...
> **CP1a / CP2a NOT written — folded into `CP3a`.** Their assertions (served-code pin, tag agreement, no-orphan grep, registration) all live in `scripts/checkpoints/cp3a-slice0-mvp-gate.sh`, which passes 6/6. Two more scripts that re-deploy and re-assert the same things would cost two extra build cycles per run for no additional coverage. Recorded here rather than left as silently-unticked boxes.
